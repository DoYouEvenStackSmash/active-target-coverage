...
def calculate_circles(screen, chain, target_point, DRAW=None):
    ...
    rad, inner_len = mfn.car2pol(
        chain.links[1].get_origin(), chain.links[1].get_endpoint()
    )
    rad2, outer_len = mfn.car2pol(
        chain.links[-1].get_origin(), chain.links[-1].get_endpoint()
    )
    ...
    pafn.draw_circle(
        screen, outer_len, chain.links[2].get_origin(), pafn.colors["cyan"]
    )
...
def rotate_two_link_chain(
    ...
    rad1, tp = tfn.calculate_rotation(
        chain.get_anchor_origin(), target_point, intermediate_point
    )
...
def check_contact(screen, A, O, VERBOSE=False):
    ...
    """
    val = find_contact(
        build_star(A.get_front_edge(), O.get_front_edge()), screen, VERBOSE
    )
...
def preprocess_circles(screen, chain, p):
    ...
    r, t = tfn.calculate_rotation(
        chain.get_anchor_origin(), chain.links[1].get_endpoint(), ps[1]
    )
        ...
        while flag != Agent.VALID:
            ...
            if len(p) and flag != Agent.VALID:
                ...
                if flag == Agent.ANGULAR:
                    v = rotate_two_link_chain(
                        screen, chain, tp1, tp2, steps=1, Olist=Olist, A=A, VERBOSE=True
                    )
                ...
                if flag == Agent.RANGE:
                    ...
                    npt = mfn.pol2car(
                        chain.links[-1].get_origin(),
                        disp,
                        chain.links[-1].get_relative_angle(),
                    )
...
def main():
    ...
    A = Agent(
        [400, 400],
        [c.links[-1].get_relative_angle(), 200, np.pi / 4],
