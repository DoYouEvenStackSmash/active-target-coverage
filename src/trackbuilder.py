...
def load_layers(files, sensing_agent):
    """
    BUILDER
...
def build_tracks(files, layer_list, sensing_agent):
    """
    Builder
...
def build_annotations(infile, outfile=None):
    ...
    layer_list = load_layers(files, sa)
    o = build_tracks(files, layer_list, sa)
    print(len(o.active_tracks))
    # sys.exit()
    freeze_tracks(o)
    # Export
    if outfile == None:
...
def main():
    ...
    draw_rot_help = (
        "draw-rot [input_loco_file] [path_to_images] [degrees (x = {90, 180, 270})]"
    )
    ...
    match command:
        ...
        case "draw-refl":
            if len(sys.argv) != 5:
                print(
                    "must specify draw-refl [input_file] [path_to_images] [axis = (x,y)]"
                )
            else:
