...
def detection_vector_test(screen):
    ...
    while 1:
        for event in pygame.event.get():
            ...
            dim = 0.15
            max_x = 1000
            det = create_detections_without_range(
                1, 1, pt[0] / max_x, pt[1] / max_x, 0.15, 0.15, 1000, 1000, np.pi, np.pi
            )
            ...
            pafn.frame_draw_bold_line(
                screen, ((500, 500), (x, 500)), pafn.colors["cyan"]
            )
            pafn.frame_draw_cross(screen, (x, 500, 0), pafn.colors["indigo"])
            pafn.frame_draw_bold_line(
                screen, ((x, 500), (x, y)), pafn.colors["magenta"]
            )
            pafn.frame_draw_bold_line(
                screen, ((500, 500), line_pt), pafn.colors["indigo"]
            )
