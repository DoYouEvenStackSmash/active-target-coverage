...
def wave_test(screen, environment, waves, measurement_rate=1):
    """
    ...
    for i in range(1,len(waves)):
      for j in range(i % measurement_rate):
        environment_agent_update(environment, True)
        # environment_agent_update(environment, True)
      if not i % measurement_rate:
        target_arr = []
        for w in range(len(waves[i])):
          target = waves[i][w]
          target_arr.append(init_target(_id = i * w + w, origin = (target[0], target[1])))
        environment.targets = target_arr
        environment.target_coverage()
        # environment_agent_update(environment, True)
...
def main():
    ...
    sa = init_sensing_agent(origin=(0, 0), width=2 * np.pi, radius =7000)
    sa.rotate_agent((500, 500))

    sa.heartbeat()
    sa.obj_tracker.avg_window_len = 4
    sa.obj_tracker.track_lifespan = 10
    sa.obj_tracker.radial_exclusion = 20
    ...
    target_arr = []
    for w in range(len(waves[0])):
      target = waves[0][w]
      target_arr.append(init_target(_id = 0 + w, origin = (target[0], target[1])))
    env.targets = target_arr
    env.target_coverage()
    # environment_agent_update(env, True)
    wave_test(screen, env, waves, int(sys.argv[-1]))
