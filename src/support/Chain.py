...
class Chain:
  ...
  def rotate_single_link(self, target_point, steps = 10, Olist = [], COLLISION_SENSITIVE = True):
    '''
  ...
  def rotate_two_link_chain(self, target_point, 
                                  intermediate_point, 
                                  steps = 10, 
                                  Olist = [], 
                                  A = None, 
    ...
    rad1,tp = tfn.calculate_rotation(self.get_anchor_origin(), target_point, intermediate_point)
  ...
  def preprocess_circles(self, p):
    ...
    r,t = tfn.calculate_rotation(self.get_anchor_origin(), self.links[1].get_endpoint(), ps[1])
  ...
  def calculate_circles(self,target_point,DRAW=None):
    ...
    rad,inner_len = MathFxns.car2pol(self.links[1].get_origin(), self.links[1].get_endpoint())
    rad2,outer_len = MathFxns.car2pol(self.links[-1].get_origin(), self.links[-1].get_endpoint())
    ...
    y = np.sqrt(np.square(target_distance) - (np.divide(np.square(np.square(inner_len) \
                + np.square(target_distance) - np.square(outer_len)), (4 * np.square(target_distance)))))
