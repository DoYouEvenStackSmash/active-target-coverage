...
def load_waves(filename):
    ...
    for anno in annos:
        ...
        fname = np.array2string(np.array(anno.imageId[:-3]).astype(str))[1:-1]
        print(fname)
        imdict[fname+suffix].append([x,y,w,h])
    return imdict
    wave_arr = []
...
def convert_back_to_world_coordinates(filename):
    ...
    global_angle = lambda x, fov_width, orientation: adjust_angle(x / 100 * fov_width - fov_width / 2 + orientation)
