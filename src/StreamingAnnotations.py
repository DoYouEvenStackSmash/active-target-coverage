...
class StreamingAnnotations:
    def register_new_yolo_annotations(
        s, valid_frame_name="frame_", w_factor=None, h_factor=None
    ):
        """
        ...
        for i in range(len(s)):
            ...
            yb = StreamingAnnotations.register_annotation(
                cbx[0], cbx[1:], f"{valid_frame_name}{len(yoloboxes)}"
            )
            yoloboxes.append(yb)
    ...
    def register_new_LOCO_annotations(annotation_arr):
        ...
        for anno in annotation_arr:
            ...
            yb = StreamingAnnotations.register_annotation(
                cbx[0], cbx[1:], valid_frame_name
            )
            yoloboxes.append(yb)

        return yoloboxes

    def register_annotation(class_id=0, bbox=None, valid_frame_name="frame_",distance=0):
        """
        Registers an annotation as a YoloBox
        """
        bbox = bbox if bbox != None else []

        yb = YoloBox(class_id, bbox, valid_frame_name,distance=distance)
        return yb
